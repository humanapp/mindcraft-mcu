...
Status: `writeBuffer` + `readBuffer` wired both VMs, host-gated (2026-06-26). Build plan:
`generated-docs/i2c-primitive-impl-plan-2026-06-26.md`.
...
- **`Buffer` I/O:** `writeBuffer` reads the arg `Buffer`'s bytes (`bufferLength`/`bufferByteAt` in TS,
  ...
  Device-API host-function to return managed bytes (the `Buffer` workstream's managed-buffer path).
...
- **`readBuffer` (I2, 2026-06-26):** returns a runtime-allocated **managed `Buffer`** - the first
  Device-API host-fn to do so (cpp `ManagedHeap::allocBuffer`, read filled directly into the buffer;
