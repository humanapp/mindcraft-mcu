...
The Mindcraft API for WODAL is adapter-shaped. It must not store device or VM execution state in process-global singletons. Each `createMicroBitV2Module()` install registers types and host functions into the `BrainServices` instance for one `MindcraftEnvironment`. Runtime device state is supplied through that brain's execution context data, currently `{ microbit }`.

Do not assume WODAL is the only Mindcraft platform loaded in the process. Stable identifiers, type IDs, and pure helper functions may live at module scope. Mutable runtime state belongs to one of these scopes:

- the `createMicroBitV2Module()` closure, when it is module configuration for one installed platform module
- the `MindcraftEnvironment` and its `BrainServices`
