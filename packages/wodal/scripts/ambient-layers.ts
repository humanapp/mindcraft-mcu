...
/** The two layer sources a platform's non-core `"wendoo"` types split into. */
export interface LayeredPlatformAmbients {
  /** wodal-general device surface, written to `wendoo.codal.d.ts`. */
  wodal: string;
  /** target-specific surface, written to `wendoo.<target>.d.ts`. */
  target: string;
}

/**
 * Splits a platform's non-core `"wendoo"` type declarations into the
 ...
 * core ambient into one `"wendoo"` module. Core-struct augmentations (the
 * `Context.microbit` field) are carried by the target layer alone.
 *
 * @param coreTypes - Registry of the core Wendoo types.
