 ...
 * async draw-image action, and the display draw / set-pixel ports. The draw port
 * line is emitted only when the display is free (a dropped draw paints nothing),
 * mirroring the C++ parity tap. Drives the display poll after each think so a
 * timed draw's hold settles and the awaiting fiber resumes on the next think.
 */
...
function buildForgetBrainJson(): LinkedBrainProgramJson {
  const rule = [
    ...
    { op: Op.JMP_IF_FALSE, a: 11 }, // relative: skip to the trailing void push
