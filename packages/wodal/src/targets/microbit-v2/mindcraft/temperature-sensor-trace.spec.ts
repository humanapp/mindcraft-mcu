...
function buildFixtureImage(environment: MindcraftEnvironment): WodalProgramImage<LinkedBrainProgram> {
...
/** Runs the committed binary over the schedule with the observable-trace taps installed. */
function runFixtureTrace(bin: Uint8Array): { trace: string; microbit: MicroBit } {
  ...
  for (const { actionId } of [MicroBitV2HostActions.Temperature, MicroBitV2HostActions.DisplaySetPixel]) {
