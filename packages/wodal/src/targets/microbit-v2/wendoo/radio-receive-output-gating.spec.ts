...
const USER_SENSOR_SOURCE = `import { Sensor, setOutput, type Context } from "wendoo";
...
function installUserSensor(environment: WendooEnvironment): { sensor: IBrainTileDef; output: BrainTileOutputDef } {
...
function getTile(environment: WendooEnvironment, tileId: string): IBrainTileDef {
  const tile = environment.brainServices.edit.tiles.get(tileId);
  assert.ok(tile, `tile '${tileId}' should be registered`);
  return tile;
}

function getOutputTile(environment: WendooEnvironment, typeId: string, name: string): BrainTileOutputDef {
...
function ruleWithSensor(environment: WendooEnvironment, sensor: IBrainTileDef): IBrainRuleDef {
