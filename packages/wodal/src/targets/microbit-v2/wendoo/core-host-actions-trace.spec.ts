...
import {
  ...
  wodalProgramBytes,
} from "../../../wendoo/program-image-binary";
...
function tile(env: WendooEnvironment, tileId: string): IBrainTileDef {
 ...
 */
function buildTimerBrainDef(env: WendooEnvironment): BrainDef {
 ...
 */
function buildBounceBrainDef(env: WendooEnvironment): BrainDef {
  const timeout = tile(env, mkSensorTileId(CoreHostActions.Timeout.key));
  const switchPage = tile(env, mkActuatorTileId(CoreHostActions.SwitchPage.key));
...
function buildImage(brainDef: BrainDef, env: WendooEnvironment): WodalProgramImage<LinkedBrainProgram> {
...
function ensureJsonGolden(jsonPath: string, build: (env: WendooEnvironment) => BrainDef): void {
