 ...
 * driver actuator's own body. Each site narrows the returned `WendooValue`
...
interface WhenResultDecl {
  /** Extra import token prefixed to the `wendoo` import (e.g. "BufferType, "). */
  imports: string;
...
function decoderSource(decl: WhenResultDecl): string {
  return `import { ${decl.imports}Sensor, type Context } from "wendoo";
...
function driverSource(decl: WhenResultDecl): string {
  return `import { ${decl.imports}Actuator, param, type Context } from "wendoo";
...
const VARIANTS: readonly Variant[] = [
  {
    ...
    producerSource: `import { Sensor, type Context } from "wendoo";
...
`,
  ...
  },
  {
    ...
    producerSource: `import { BufferType, Sensor, type Context } from "wendoo";
...
function buildImage(environment: WendooEnvironment, variant: Variant): WodalProgramImage<LinkedBrainProgram> {
...
const PARENT_PRODUCER_SOURCE = `import { Sensor, type Context } from "wendoo";
...
const CHILD_PRODUCER_SOURCE = `import { Sensor, type Context } from "wendoo";
