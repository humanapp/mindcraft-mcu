...
async function makeProductionShapedHost(): Promise<{
  ...
  const projectStore = await createIdbProjectStore(`mindcraft-import-targets-${storeCounter++}`);
...
const MISSING_PLATFORM_COORDINATE = "mindcraft-lang/lib-missing-platform";
...
/**
 * A shared `.mindcraft` export document whose manifest declares the embedded
...
async function importTargetedProject(documentText: string): Promise<{
  ...
  const file = new File([documentText], "targeted.mindcraft");
...
describe(".mindcraft import carries the manifest targets declaration -- production wiring", () => {
