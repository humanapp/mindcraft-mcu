...
function renderedReadoutValue(instance: SimulatorInstance): string {
  const markup = renderToStaticMarkup(createElement(TemperatureSlider, { instance }));
  const match = markup.match(/data-testid="temperature-value"[^>]*>(-?\d+)</);
  assert.ok(match, "expected a temperature numeric readout carrying a value");
  return match[1];
}
...
describe("TemperatureSlider", () => {
  ...
  test("a fresh instance renders the readout at the device default of 21", () => {
    assert.equal(renderedReadoutValue(instanceWith(new MicroBit())), "21");
  });

  test("the readout reflects a negative device model temperature", () => {
    const microbit = new MicroBit();
    microbit.setTemperature(-5);
    assert.equal(renderedReadoutValue(instanceWith(microbit)), "-5");
  });

  test("the readout reflects the device model's current temperature at the range top", () => {
    const microbit = new MicroBit();
    microbit.setTemperature(50);
    assert.equal(renderedReadoutValue(instanceWith(microbit)), "50");
  });
});
